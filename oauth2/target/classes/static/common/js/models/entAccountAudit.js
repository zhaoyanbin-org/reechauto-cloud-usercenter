        ...
        render : function () {
            ...
            table.render({
                ...
                ,cols :[[{
                    ...
                    title: "更新时间",
                    templet:function (row) {
                        return row.createTime ? new Date(row.updateTime).format("yyyy-MM-dd hh:mm:ss"):"";
                    }
                },{
                    field: "createTime",
                    title: "创建时间",
                    templet:function (row) {
                        return row.updateTime ? new Date(row.createTime).format("yyyy-MM-dd hh:mm:ss"):"";
                    }
                },{
                    field: "op",
                    title: "操作",
            ...
            form.on('submit(query)', function(data){
                table.reload("account-table",{
                    url:"/api/entAccount/getVerfiyList",
                    method:"post",
                    where:{
...
//                    }
                    layui.layer.open({
                        ...
                        , success: function (layero, index) {
                            ...
                            $.get('/api/entAccount/getFileIdList?entId='+data.id,null, function(data) {
                                if(data.code == 0){
                                    data =data.data;
                                    var html = [];
                            ...
                            $('#account-audit-layer input[name="name"]').val(data.name);
                        ...
                        , yes: function (index, layero) {
                            ...
                            layui.layer.msg('您确定通过审核吗？', {
                                ...
                                ,btn: ['确定', '取消']
                                ,yes: function(index){
                                    $.post("/api/entAccount/doAuth", { entId: data.id, reason: "审核通过",status : 1 }, function(data) {
                                        if(!data){
                        ...
                        , btn2: function (index, layero) {
                            ...
                            layui.layer.prompt({title: '请输入不通过原因', formType: 2}, function(reason, index){
                                ...
                                $.post("/api/entAccount/doAuth", { entId: data.id, reason: reason,status : 2 }, function(data) {
                                    if(!data){
                                        layui.layer.msg('操作失败', {icon: 2});
                                    }else if(data.code == 0 ){
    ...
    exports('entAccountAudit', function () {
    	entAccountAudit.init();
    });
});