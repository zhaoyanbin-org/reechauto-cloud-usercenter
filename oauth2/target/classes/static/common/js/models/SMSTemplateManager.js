...
layui.define(["table","form"],function(exports){
    ...
    var form = layui.form;
    var fakeData =[
        {templateId:1, name:"测试数据1", appKey:"summertest", status:"enable", subject:"summertest", content:"summertest", createTime:"2017-12-20", updateTime:"2017-12-20"},
        {templateId:2, name:"测试数据2", appKey:"summertest", status:"enable", subject:"summertest", content:"summertest", createTime:"2017-12-20", updateTime:"2017-12-20"}
    ];
    var SMSTemplateManager = {
        init : function () {
            this.render();
        },
        render : function () {
            ...
            $("#sms-template-new-btn").on("click",function () {
                layui.layer.open({
                    type : 1
                    ,title: "新增模板" //不显示标题栏
                    ,area:  ['800px', '600px']
                    ...
                    ,yes: function(index, layero){
                        ...
                        if(!id){
                            layui.layer.msg("请输入模板ID，由字母、数字、下划线组成，如account_sms_invite_zh_CN", {icon: 2,time:3000});
                            return false;
                        }
                        if(!name){
                        ...
                        $.post('/api/colibri/smsTemplate/add',{
                            id : id,
                            name : name,
                            appKey : appKey,
                            content : content
            ...
            form.on('submit(query)', function(data){
                table.reload("template-table",{
                    url:"/api/colibri/smsTemplate/queryList",
                    method:"get",
                    where:{
                        id:data.field.query,
            ...
            table.on('tool(templateTable)', function(obj){
                ...
                if(obj.event == 'disableTemplate'){
                    layui.layer.msg('您确定要停用该模板吗['+data.id+']吗？', {
                        ...
                        ,btn: ['确定', '取消']
                        ,yes: function(index){
                            $.post("/api/colibri/smsTemplate/modify",{
                                id : data.id,
                                status : "disable"
                            },function(data){
                                if(!data || data.code != 0 ){
                    ...
                    layui.layer.open({
                        ...
                        ,success: function (layero, index) {
                            $.ajax({
                                type: "GET",
                                url:"/api/colibri/smsTemplate/query?id="+data.id,
                                cache: false,
                                dataType: "json",
                                async: false,
    ...
    exports('SMSTemplateManager', function () {
        SMSTemplateManager.init();
    });
});